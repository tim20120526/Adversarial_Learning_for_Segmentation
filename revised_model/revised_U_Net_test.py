...
import cv2
import os
class Revised_Unet_test:
    def __init__(self):
        print('Revised U-net Network test')
    ...
    def train(self, x_train, y_train, x_valid, y_valid, x_test, y_test, batch_size, model_dir, tb_dir):
    ...
    def pred(self, x_train, y_train, x_valid, y_valid, x_test, y_test, name_list, batch_size, model_dir,PREDICT_SAVED_DIRECTORY):
        gpuConfig = tf.ConfigProto(allow_soft_placement=True)
        gpuConfig.gpu_options.allow_growth = True
        self.loss_fuc(batch_size)
        tf.summary.scalar("loss", self.loss_mean)
        ...
        with tf.Session(config=gpuConfig) as sess:  # 开始一个会话
            ...
            for x, y in generator(x_test, y_test, batch_size, x_test.shape[0], False):
                ...
                #                print(cnt)
                #                print(len(name_list))
                cv2.imwrite(os.path.join(PREDICT_SAVED_DIRECTORY, '{}_pred.jpg'.format(name_list[cnt])), image[0] * 255)
                cnt += 1
                test_acc = sum_acc / x_test.shape[0] * batch_size
            #                    test_los = sum_los/x_test.shape[0]
